...
meta <- data.frame(
  ...
  # `top_nt` is required by `signed = TRUE` (and by `excluded_nts`); we set
  # it here so the signed-mode example below can run.
  top_nt    = c("acetylcholine", "gaba", "acetylcholine", "glutamate", "acetylcholine")
)
...
```{r plot_results, fig.width=10, fig.height=6, fig.alt="Bar chart comparing influence scores from neurons A and B across target neurons C, D, and E. Shows that neuron A has higher influence on targets C and D, while neuron B has higher influence on target E."}
...
```{r signed_network}
# Create calculator with signed connectivity. The library has no per-organism
# default for which transmitters are inhibitory; you must supply
# `inhibitory_nts` explicitly when `signed = TRUE`. (`excluded_nts` is also
# accepted, for transmitter classes that gate both excitatory and inhibitory
# receptors and so cannot be assigned a single sign safely.)
